...
void uvec_test_base(void) {
    ...
    utest_assert_ok(uvec_push(VTYPE, &v, 4));
    ...
    utest_assert_ok(uvec_insert_at(VTYPE, &v, 2, 4));
...
void uvec_test_range(void) {
    ...
    // uvec_set_range
    utest_assert_ok(uvec_set_range(VTYPE, &v, items, 3, ulib_array_count(items)));
    ...
    uvec_clear(VTYPE, &v);
    uvec_append_items(VTYPE, &v, 1, 2, 3, 4, 5);
    utest_assert_ok(uvec_insert_range(VTYPE, &v, items, 2, ulib_array_count(items)));
...
void uvec_test_capacity(void) {
    ...
    utest_assert_ok(uvec_reserve(VTYPE, &v, capacity));
    ...
    utest_assert_ok(uvec_expand(VTYPE, &v, capacity));
...
void uvec_test_storage(void) {
    ...
    utest_assert_ok(uvec_push(VTYPE, &vec, 4));
    utest_assert_uint(uvec_count(VTYPE, &vec), ==, 4);
    uvec_assert_elements(VTYPE, &vec, 1, 2, 3, 4);

    utest_assert_ok(uvec_reserve(VTYPE, &vec, 1000));
    utest_assert_ok(uvec_shrink(VTYPE, &vec));
...
void uvec_test_equality(void) {
    ...
    utest_assert_ok(uvec_copy(VTYPE, &v1, &v2));
    ...
    utest_assert_ok(uvec_push(VTYPE, &v2, 5));
    utest_assert_false(uvec_equals(VTYPE, &v1, &v2));
...
void uvec_test_contains(void) {
    ...
    utest_assert_ok(uvec_push_unique(VTYPE, &v1, 7));
    uvec_assert_elements(VTYPE, &v1, 3, 2, 5, 4, 5, 1, 7);
    utest_assert(uvec_contains(VTYPE, &v1, 7));

    utest_assert_ret(uvec_push_unique(VTYPE, &v1, 7), ULIB_NO);
    uvec_assert_elements(VTYPE, &v1, 3, 2, 5, 4, 5, 1, 7);
...
void uvec_test_comparable(void) {
    ...
    utest_assert_ok(uvec_append(VTYPE, &v, &values));
    ...
    uvec_foreach (VTYPE, &values, loop) {
        if (!uvec_contains(VTYPE, &v, *loop.item)) {
            utest_assert_ok(uvec_push(VTYPE, &v, *loop.item));
        }
    }
    ...
    utest_assert_ok(uvec_sorted_unique_insert(VTYPE, &v, 7, &idx));
    uvec_assert_elements(VTYPE, &v, 0, 1, 2, 3, 3, 5, 6, 7);
    utest_assert_uint(idx, ==, 7);

    utest_assert_ret(uvec_sorted_unique_insert(VTYPE, &v, 3, &idx), ULIB_NO);
    uvec_assert_elements(VTYPE, &v, 0, 1, 2, 3, 3, 5, 6, 7);
    utest_assert_uint(idx, ==, 3);
...
void uvec_test_max_heapq(void) {
    ...
    for (unsigned i = 0; i < ulib_array_count(arr); ++i) {
        utest_assert_ok(uvec_max_heapq_push(VTYPE, &heap, arr[i]));
...
void uvec_test_min_heapq(void) {
    ...
    for (unsigned i = 0; i < ulib_array_count(arr); ++i) {
        utest_assert_ok(uvec_min_heapq_push(VTYPE, &heap, arr[i]));
        utest_assert_uint(uvec_count(VTYPE, &heap), ==, i + 1);
