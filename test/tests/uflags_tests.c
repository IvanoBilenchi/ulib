...
#define uflags_test_impl(N) {                                                                       \
    ...
    utest_assert(flags == 0);                                                                       \
    utest_assert(uflags_count_set(N, flags) == 0);                                                  \
    utest_assert(uflags_count_unset(N, flags) == N);                                                \
                                                                                                    \
    ...
    flags = uflags_all(N);                                                                          \
    utest_assert(flags != 0);                                                                       \
    utest_assert(uflags_count_set(N, flags) == N);                                                  \
    utest_assert(uflags_count_unset(N, flags) == 0);                                                \
                                                                                                    \
    for (unsigned i = 0; i < N; ++i) {                                                              \
        utest_assert(uflags_is_set(N, flags, uflags_bit(N, i)));                                    \
    }                                                                                               \
    ...
    utest_assert(flags != 0);                                                                       \
    utest_assert(uflags_count_set(N, flags) == 1);                                                  \
    utest_assert(uflags_count_unset(N, flags) == (N - 1));                                          \
                                                                                                    \
    for (unsigned i = 0; i < N; ++i) {                                                              \
        if (i == (N - 2)) {                                                                         \
            utest_assert(uflags_is_set(N, flags, uflags_bit(N, i)));                                \
    ...
    utest_assert(flags != 0);                                                                       \
    utest_assert(uflags_count_set(N, flags) == 2);                                                  \
    utest_assert(uflags_count_unset(N, flags) == (N - 2));                                          \
                                                                                                    \
    for (unsigned i = 0; i < N; ++i) {                                                              \
        if (i == 1 || i == (N - 2)) {                                                               \
            utest_assert(uflags_is_set(N, flags, uflags_bit(N, i)));                                \
    ...
    utest_assert(!uflags_is_any_set(N, flags, uflags_bit(N, 2) | uflags_bit(N, (N - 1))));          \
                                                                                                    \
