...
bool uhash_test_memory(void) {
    ...
    utest_assert_uint(set->n_buckets, ==, buckets);
    utest_assert_uint(uhash_count(set), ==, 0);
...
bool uhash_test_base(void) {
    ...
    utest_assert_uint(uhash_count(set), ==, MAX_VAL);
